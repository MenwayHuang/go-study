...
本项目 (ch01-ch10) 的定位：
  ...
  │                     ┌──────────┐                            │
  │                     │ 生产实战  │  ← 还没到这里              │
  │                     │K8s/Docker│    (做完本项目后的下一步)    │
  │                     │可观测/压测│                            │
  │                  ┌──┴──────────┴──┐                         │
  │                  │  架构设计思想   │  ← ch06/ch07/ch09 初步涉及│
  │                  │ 微服务/高可用   │    (本项目有原理和流程图)  │
  │               ┌──┴────────────────┴──┐                      │
  │               │  核心基础知识 ★当前   │  ← ch01~ch10 主要在这里│
  │               │高并发/高可用/高性能原理│    (原理 + 内存模拟实现) │
  │            ┌──┴──────────────────────┴──┐                   │
  │            │     Go 语言基础             │  ← 前置知识       │
  │            │  语法/类型/接口/错误处理     │                   │
  │            └────────────────────────────┘                   │
  ...
  本项目 = 核心基础知识层（用纯Go实现，理解原理）
  后续计划 = 生产实战层（真实框架 + 真实中间件 + 真实部署）
```
...
| 章节 | 主题 | 分类 | 核心知识点 | 依赖 | 难度 |
|------|------|------|-----------|------|------|
| [第1章](./ch01-concurrency-basics/) | Go并发基础 | 🔵高并发基础 | goroutine、channel、sync原语、context | 无 | ⭐⭐ |
| [第2章](./ch02-concurrency-patterns/) | 并发模式 | 🔵高并发模式 | Worker Pool、Fan-out/Fan-in、Pipeline、限流器 | 无 | ⭐⭐⭐ |
| [第3章](./ch03-high-concurrency-http/) | 高并发HTTP服务 | 🔵高并发 + 🟢高可用 | 优雅关闭、超时控制、中间件链 | 无 | ⭐⭐⭐ |
| [第4章](./ch04-message-queue/) | 消息队列解耦 | 🔵高并发(削峰) + 🏗️架构 | 内存MQ、Pub/Sub、异步处理 | 无 | ⭐⭐⭐ |
| [第5章](./ch05-cache-strategy/) | 缓存策略 | 🔴高性能 + 🔵高并发 | 本地缓存、LRU、穿透/雪崩/击穿、SingleFlight | 无 | ⭐⭐⭐ |
| [第6章](./ch06-microservice/) | 微服务架构 | 🏗️架构 + 🟢高可用 | 服务注册发现、负载均衡、熔断器、链路追踪 | 无 | ⭐⭐⭐⭐ |
| [第7章](./ch07-high-availability/) | 高可用设计 | 🟢高可用 | 重试策略、优雅降级、幂等设计、健康检查 | 无 | ⭐⭐⭐⭐ |
| [第8章](./ch08-performance/) | 性能优化 | 🔴高性能 | pprof、sync.Pool、内存优化、锁优化 | 无 | ⭐⭐⭐⭐ |
| [第9章](./ch09-seckill-system/) | 实战：秒杀系统 | 🔵🟢🔴综合实战 | 综合运用全部知识 + 内置压测 | 无 | ⭐⭐⭐⭐⭐ |
| [第10章](./ch10-middleware-practice/) | 中间件实战 | 🏗️架构 + 生产入门 | Redis缓存/锁/计数器 + NATS消息队列 | **Docker** | ⭐⭐⭐⭐ |

**分类图例**：🔵高并发  🟢高可用  🔴高性能  🏗️架构设计
...
> 详细的后续学习路线和选型建议见 [PRODUCTION-ROADMAP.md](./PRODUCTION-ROADMAP.md)

1. **读源码**: `net/http` 标准库、`sync.Pool` 实现（附源码阅读方法论）
2. **框架实战**: 用 Iris/Gin + gRPC 构建真实微服务
3. **真实中间件**: Redis + RabbitMQ/NATS + MySQL
4. **压测工具**: wrk / vegeta / k6 进行真正的性能测试
5. **可观测性**: Jaeger 链路追踪 + Prometheus + Grafana 监控
6. **部署**: Docker + K8s 部署微服务集群
