...
### 10.1.1 Redis 分布式锁原理详解

```
为什么需要分布式锁？

单机:
  sync.Mutex 就够了，同一进程内互斥

多实例部署:
  实例A ──→ sync.Mutex ──→ 处理订单123
  实例B ──→ sync.Mutex ──→ 也在处理订单123!  ← 两个实例各自的锁互不影响
